...
def runSimulation(timeline_number):
    ...
    while ((counter <= simulation_limit) & (sumMatrixValues(timeline) != 0) & (
        ...
        # Seguindo o RND, calculamos um local aleatorio onde a mensagem entrara, diferente do FIFO que entra no topo
        timeline[next_simulation_event[1]][np.random.randint(0, posts_number)] = next_simulation_event[2]
